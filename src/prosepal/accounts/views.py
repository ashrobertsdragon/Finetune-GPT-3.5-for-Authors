...
from .utils import (
    get_binders,
    initialize_user_db,
    preload_account_management_form,
...
def profile_view():
    ...
    if password_form.validate_on_submit():
        ...
        try:
            response = auth.update_user({"password": new_password})
...
def buy_credits_view():
    ...
    if form.validate_on_submit():
        ...
        return redirect(
            url_for("stripe.checkout_view", num_credits=num_credits)
        )
...
def sort_binders_view():
    ...
    binders = sorted(
        binders_list, key=lambda x: x[column], reverse=sort_descending
    )
